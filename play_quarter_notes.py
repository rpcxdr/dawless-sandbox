...
def main() -> None:
    ...
    def play_loop() -> None:
        current_audio = audio_up
        current_offset = 0
        last_direction = is_ascending
        chunk_size = 2048

        while not stop_event.is_set():
            direction = is_ascending
            if direction != last_direction:
                current_audio = audio_up if direction else audio_down
                current_offset = 0
            ...
            chunk = current_audio[current_offset:current_offset + chunk_size]
            if len(chunk) == 0:
                current_offset = 0
                continue
